...
export class CheckinService {
  ...
  async recordCheckIn(userId: string, ipAddress?: string, userAgent?: string) {
    ...
    // Emit realtime event to notify frontend
    this.eventsGateway.emitToRoom(
      this.getUserTopic(userId),
      'checkin:recorded',
      {
    ...
    // If alerts were cancelled, notify
  ...
  /**
   * Get user-specific topic for Socket.IO room
   */
  private getUserTopic(userId: string): string {
    return `user_${userId}`;
  }
}
